...
public class ThumbnailsAccessor {
    ...
    public boolean writeItemThumbnail(EuropeanaItem item, OutputStream out, boolean useTypeThumb)
        ...
        if (!bOk)
            bOk = this.http.silentWriteURLContent(item.getOriginalThumbnailURL(), out, mime);
        if (!bOk && useTypeThumb)
    ...
    public List<EuropeanaItem> copyThumbnails(EuropeanaQuery search, File dir, int maxResults)
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if ((id != null) && (id.length() > 0)) {
                ...
                try {
                    if (i % 10 == 0)
                        Log.i("europeana", "Copying thumbnail: " + (i + 1) + " / " + items.size());
        ...
        Log.i("europeana", "Copying finished OK, thumbnails generated: " + itemsOk.size());
        return itemsOk;
    }
